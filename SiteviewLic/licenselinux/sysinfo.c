...
void GetMaxDate(char* CurDate)
{
 ...
 //  printf ("MaxDate :  %s\n\r",MaxDate);
  ...
  // printf ("MaxDate :  %s\n\r",MaxDate);
	...
	time(&secs_now);  
	time_now   =   localtime(&secs_now);  
	strftime(MaxDate, 16, "%Y%m%d", time_now);
   #endif
...
int GetInstallDate(char* insdate)
{
	...
	if(getcwd(buf, sizeof(buf)-1) != NULL)
	{
	    if ((res = stat (buf, &fstat)) != 0)
	    {   
		strcat(insdate, "0000000000000000");
	    }
	    else
	    {	   
		time_now = localtime (& fstat. st_ctime); 
		//strftime(buf, 16, "IST:%Y%m%d%H%M%S", time_now);  
		strftime(buf, 16, "%Y%m%d", time_now); 
		strcpy(insdate, buf);
		return true;		    
	    }
	}
