...
int  WriteDataDirFromSysReg(char *lpSubKey,char *lpValueName,char *RegData)
{
	...
	{
	      reg.CreateKey(HKEY_LOCAL_MACHINE,lpSubKey,"");
		  if (!reg.Open(HKEY_LOCAL_MACHINE,lpSubKey)) 
		  {  
			  return false;	
		  }
	}
	if (reg.WriteString(lpValueName,RegData,&pdwLastError))  
	{
		reg.Close();
		return true;
	}
...
int  ReadDataDirFromSysReg(char *lpSubKey,char *lpValueName,char *RegData)
{
	...
	if (reg.ReadString(lpValueName,bufData,1024,&pdwLastError))  
	{
		strcpy(RegData,bufData);
		reg.Close();
		return true;
	}
	reg.Close();
