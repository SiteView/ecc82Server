...
CString g_startItsm	= "startofbiz.bat";
CString g_stopItsm	= "stopofbiz.bat";
...
CString g_startWeb813	= "startofbiz.bat";
CString g_stopWeb813	= "stopofbiz.bat";
	...
	switch(opcode)
	{
		...
		//	::SetEvent(g_hCom);
		   ...
		   ::PostThreadMessage(g_alertpi.dwThreadId,WM_QUIT,0,0);
			...
			//RunExecuteBat(rootPath+"\\ofbiz\\", g_stopItsm);	// 启动 itsm目录中的 stop.bat
...
void WINAPI SVS_ServiceStart(DWORD argc, LPTSTR *argv) 
{
	...
	//RunExecuteBat(rootPath+"\\ofbiz\\", g_startItsm);	
...
DWORD WINAPI WatchWeb813(LPVOID lpParam )
{
	 while(true)
	 {
		...
		else
		{
			...
			RunExecuteBat(rootPath+"\\ofbiz\\", g_stopWeb813);	
			Sleep(20*1000);
			// 启动 WebECC8.1.3目录中的 start.bat
			//RunExecuteBat(rootPath+"\\WebECC8.1.3\\", g_startWeb813);
			RunExecuteBat(rootPath+"\\ofbiz\\", g_startWeb813);
			//Sleep(20*1000);
		}
...
DWORD WINAPI WatchOfbizService(LPVOID lpParam )
{
	 while(true)
	 {
		...
		else
		{
			...
			RunExecuteBat(rootPath+"\\ofbiz\\", g_stopItsm);	
			Sleep(20*1000);
			// 启动 itsm目录中的 start.bat
			RunExecuteBat(rootPath+"\\ofbiz\\", g_startItsm);	
			//Sleep(20*1000);
