...
/*
  ...
  Peter E. Mellquist
*/
char snmpget_cpp_version[]="@(#) SNMP++ $Id: snmpGet.cpp 264 2006-06-16 20:53:15Z fock $";
...
int main(int argc, char **argv)
{
   ...
   Oid oid("1.3.6.1.2.1.1.1.0");      // default is sysDescr
	//Oid oid("1.3.6.1.2.1.25.2.3.1.1");
   ...
   OctetStr community("public1");                   // community name
   ...
   for(int x=1;x<argc;x++) {                           // parse for version
     ...
     if ( strstr( argv[x],"-auth") != 0) {
       ptr = argv[x]; ptr+=5;
       if (_stricmp(ptr, "SHA") == 0)
	   authProtocol = SNMP_AUTHPROTOCOL_HMACSHA;
       else if (_stricmp(ptr, "MD5") == 0)
   ...
   if (status == SNMP_CLASS_SUCCESS)
   {
	 ...
	 cout << "ValueX = " << vb.iv_vb_value->get_printable() << endl;
