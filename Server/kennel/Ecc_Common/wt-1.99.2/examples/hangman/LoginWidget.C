   ...
   WContainerWidget(parent)
{
   ...
   WText *title = new WText(L"Login", this);
   title->decorationStyle().font().setSize(WFont::XLarge);

   IntroText =
      new WText(L"<p>Hangman keeps track of the best players. To recognise "
   ...
   WLabel *usernameLabel = new WLabel(L"User name: ", layout->elementAt(0, 0));
   ...
   WLabel *passwordLabel = new WLabel(L"Password: ", layout->elementAt(1, 0));
   Password = new WLineEdit(layout->elementAt(1, 1));
   Password->setEchoMode(WLineEdit::Password);
   passwordLabel->setBuddy(Password);

   WLabel *languageLabel = new WLabel(L"Language: ", layout->elementAt(2, 0));
   Language = new WComboBox(layout->elementAt(2, 1));
   Language->insertItem(0, L"English words (18957 words)");
   Language->insertItem(1, L"Nederlandse woordjes (1688 woorden)");
   languageLabel->setBuddy(Language);

   new WBreak(this);

   WPushButton *LoginButton = new WPushButton(L"Login", this);
...
void LoginWidget::checkCredentials()
{
   ...
   if (HangmanDb::validLogin(User, pass)) {
      ...
      IntroText
	->setText(L"<p>You entered the wrong password, or the username "
...
void LoginWidget::confirmLogin(const std::wstring text)
{
   ...
   WText *title = new WText(L"Loging successful", this);
   title->decorationStyle().font().setSize(WFont::XLarge);

   new WText(text, this);
   (new WPushButton(L"Start playing", this))
