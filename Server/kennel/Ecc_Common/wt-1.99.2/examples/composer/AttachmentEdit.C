    ...
    taken_(false)
{
  ...
  remove_->setMargin(WLength(5, WLength::Pixel), Left);
  remove_->clicked.connect(SLOT(this, WWidget::hide));
  remove_->clicked.connect(SLOT(this, AttachmentEdit::remove));
  ...
  uploaded_ = new WText(L"", this);
  uploaded_->setStyleClass(L"option");
  uploaded_->hide();

  // The error message.
  error_ = new WText(L"", this);
  error_->setStyleClass(L"error");
...
void AttachmentEdit::uploaded()
{
  if (!upload_->emptyFileName()) {
    ...
    uploaded_->setText(escapeText(fileName_)
